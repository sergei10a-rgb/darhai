 ...
 * parsing the electron-log file for [Wayland:ready] / [Wayland:init] /
 * [Wayland:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [Wayland:ready] marks
  ...
  // Parsed from [Wayland:init] marks
  initTotalMs: number;
  // Parsed from [Wayland:process] marks
...
// Matches: [Wayland:ready] <label> +<ms>ms
// Matches: [Wayland:init]  <label> +<ms>ms
// Matches: [Wayland:process] <label> +<ms>ms
const MARK_REGEX = /\[Wayland:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[Wayland] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[Wayland] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[Wayland] Showing main window')) marks.logs.showingMainWindow = true;
  }
