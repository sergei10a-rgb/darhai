...
function resolvePackagedApp(projectRoot) {
  ...
  // 'Darhai' is the current executableName; 'Wayland' kept as a fallback for
  // stale pre-rebrand out/ directories.
  if (process.platform === 'win32') {
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      for (const name of ['Darhai.exe', 'Wayland.exe']) {
    ...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      for (const name of ['Darhai', 'Wayland']) {
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['Darhai', 'darhai', 'wayland', 'Wayland']) {
...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('Darhai.exe');
    await killProcessByName('Darhai');
    // Stale pre-rebrand processes
    await killProcessByName('Wayland.exe');
    await killProcessByName('Wayland');
  ...
  const env = {
    ...process.env,
    DARHAI_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] DARHAI_EXTENSIONS_PATH: ${env.DARHAI_EXTENSIONS_PATH}`);
