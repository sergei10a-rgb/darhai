...
The pr-review skill must have been executed in the current session. The review report (containing a "Summary" table) must be present in the conversation.
...
Locate the **Summary** section in the review report:
...
If the Summary table is empty, abort with:
...
If the issue is real, evaluate the review report's "Fix Recommendation":
...
⚠️ Triage found CRITICAL issue #<issue_number> may be a false positive, but the automated workflow cannot dismiss CRITICAL-level issues on its own - escalating to human review.
...
> Triage believes CRITICAL issue #N may be a false positive: <reason>
...
> Pushed to `<head_branch>` - PR #<PR_NUMBER> has been automatically updated. No new PR needed.
...
The original PR is from a fork without maintainer push access, so fixes cannot be pushed directly.
A follow-up PR #${NEW_PR_NUMBER} has been created in the main repo containing all fixes from this review - it will auto-merge once CI passes."
```
...
> Fork PR has no maintainer push access - a follow-up PR #<NEW_PR_NUMBER> has been created in the main repo and will auto-merge once CI passes.
...
Post the verification report as a PR comment AND output it in the conversation. The report now includes a **Triage Decisions** section before the fix table:
...
**Fix method:** Pushed directly to `<head_branch>`
...
| 3 | 🟡 MEDIUM | <issue description> | 🔄 Alternative | <why the original suggestion doesn't apply and what the alternative is> |

> Only lists issues that were dismissed (DISMISSED) or resolved with an alternative (FIX_ALT). Issues where the original suggestion was adopted (FIX) are not shown here.
> If all issues adopted the original suggestion, omit this section.

### Fix Results

| # | Severity | File | Issue | Fix Applied | Status |
|---|---------|------|-------|-------------|--------|
| 1 | 🔴 CRITICAL | `file.ts:N` | <original issue> | <fix applied> | ✅ Fixed |
| 2 | 🟠 HIGH     | `file.ts:N` | <original issue> | <fix applied> | ✅ Fixed (alternative) |
| 3 | 🟡 MEDIUM   | `file.ts:N` | <original issue> | - | ⏭️ Dismissed |

**Summary:** ✅ Fixed N | 🔄 Alternative N | ⏭️ Dismissed N | ❌ Could not fix N

> 🔵 LOW severity issues were skipped (not merge-blocking; low fix priority).
EOF
)"
```
...
```
 ...
 9. Verify → post Triage Decisions + Fix Results as gh pr comment + output in conversation
