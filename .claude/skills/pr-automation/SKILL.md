...
✅ Automatically fixed. No blocking issues found in the code - please confirm and merge."
```
...
The following jobs did not pass during this automated review - please fix them:

| Job | Conclusion |
|-----|------------|
| <failed job name> | ❌ <FAILURE or CANCELLED> |

Automated review is paused. This PR will be re-processed once all CI checks pass."
```
...
This PR has conflicts with the target branch, and the automatic rebase could not resolve them cleanly. Please rebase manually and push again:
...
1. Post comment:
   ...
   if [ -n "$CRITICAL_FILES" ]; then
     ...
     CRITICAL_SECTION="
   > 📂 **Files matching the critical path:**
   ${CRITICAL_LIST}"
   else
     CRITICAL_SECTION=""
   fi
   ...
   gh pr comment <PR_NUMBER> --body "<!-- pr-automation-bot -->
   ✅ Automated review complete. No blocking issues found - please confirm and merge.${CRITICAL_SECTION}"
   ```
...
1. Post comment:
   ```bash
   gh pr comment <PR_NUMBER> --body "<!-- pr-automation-bot -->
   ❌ This PR has blocking issues and cannot be processed automatically. Transferred to human review. See the review report above for details."
   ```
2. Update labels:
   ```bash
