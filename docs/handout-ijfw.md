...
| **Дархайн `constitutionBridge.ts` (2 газар)** | ✅ **зассан** (`8d4027e08`) |
| **Дархайн `audit-log.ts` лог эргэлт** | ✅ **зассан** — ENOENT л no-op, түгжээнд copy+truncate |
| `ijfwDropBridge.ts` карантин | ✅ **зассан** — `moveFileAtomic` |
| `LocalFileBackend.ts` (шифрлэгдсэн sync blob) | ✅ **зассан** — тогтмол `.tmp` нэр арилсан |
| Ерөнхий `writeFileAtomic` туслах функц | ✅ аль хэдийн байсан — **EPERM fallback нь дутуу байсныг нэмэв** |

### Юу яг болсон (2026-08-07)

Туслах функц (`src/process/utils/atomicWrite.ts`) **аль хэдийн байсан** ч яг гол
зүйл нь дутуу байв: rename бүтэлгүйтэхэд tmp-г цэвэрлээд **алдаа шиднэ**. Өөрөөр
хэлбэл түүний 5 дуудагч бүгд Windows дээр бичилтээ алддаг байсан.

**Энэ машин дээр өөрөө хэмжсэн** (win32 10.0.26200 / Node 24): зорилтот файлыг
зөвхөн уншихаар нээж барихад `renameSync` → **EPERM**, тэр ЯГ ТЭР handle дээр
`copyFileSync` → **амжилттай**. Гарын авлагын мэдэгдэл батлагдав.

Тиймээс `EPERM`/`EBUSY`/`EACCES`-ыг (өмнөх `EXDEV`-ийн хажууд) copy+unlink рүү
чиглүүлэв. `ENOSPC` зэрэг хамаагүй код хэвээр шидэгдэнэ — зай дүүрсэн үед хуулах
нь тэр зайг л иднэ.

**19 тест**, хоёр мутациар нотолсон: fallback-ыг устгавал 5 тест унана;
audit-log-ийн бүхнийг залгидаг `catch {}`-ыг буцаавал 1 тест унана.

**Үлдсэн (энэ ангилалд БИШ, шалгасан):** `HubInstaller.ts` (фолдер зөөдөг —
copy fallback нь өөр зан төлөв шаардана), `database/index.ts` (эвдэрсэн санг
нөөцлөх — унших замгүй байх нөхцөлд ажилладаг).
