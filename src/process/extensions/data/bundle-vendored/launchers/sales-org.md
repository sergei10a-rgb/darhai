...
2. Most recent input line 1 is exactly `[DARHAI_CRON_FIRE:sales-org]` → weekly check-in.
   2a. **Cron mid-welcome-back.** If cron fires while welcome-back is pending (last assistant message ended with `Pick up where we left off, or point us somewhere new?` AND new input line 1 is `[DARHAI_CRON_FIRE:sales-org]`), run the weekly check-in (Rule 2), then append: _"Welcome-back still pending - pick up where we left off, or fresh direction?"_
...
Scan the list for any cron whose `message` line 1 is `[DARHAI_CRON_FIRE:sales-org]`. If found, skip - Company already standing. Else proceed.
...
message: [DARHAI_CRON_FIRE:sales-org]
