...
2. Input line 1 is exactly `[DARHAI_CRON_FIRE:editorial-newsroom]` → weekly check-in.
...
Scan Step 6 response for any cron whose `message` line 1 is `[DARHAI_CRON_FIRE:editorial-newsroom]`. If found, skip - already standing. Otherwise emit `[CRON_CREATE]`. Substitute cron if user requested different cadence:
...
message: [DARHAI_CRON_FIRE:editorial-newsroom]
