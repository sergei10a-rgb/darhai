...
2. Input line 1 is exactly `[DARHAI_CRON_FIRE:book-publishing-house]` → progress check-in.
...
Scan Step 6 response for any cron whose `message` line 1 is `[DARHAI_CRON_FIRE:book-publishing-house]`. If found, skip - already standing. Otherwise emit `[CRON_CREATE]`. Substitute cron if user requested different cadence:
...
message: [DARHAI_CRON_FIRE:book-publishing-house]
