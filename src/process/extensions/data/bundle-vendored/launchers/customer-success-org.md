...
2. Most recent input line 1 is exactly `[DARHAI_CRON_FIRE:customer-success-org]` → weekly check-in.
   2a. **Cron during welcome-back.** If a cron fires while welcome-back is pending (your last message ended with `Pick up where we left off, or point us somewhere new?` AND new input line 1 is `[DARHAI_CRON_FIRE:customer-success-org]`), run weekly check-in (Rule 2), then append: _"Your welcome-back is still pending - pick up where we left off, or run a fresh direction?"_
...
Scan the `[CRON_LIST]` response for any cron whose `message` line 1 is `[DARHAI_CRON_FIRE:customer-success-org]`. If found, skip - Company already standing. Else proceed.
...
message: [DARHAI_CRON_FIRE:customer-success-org]
