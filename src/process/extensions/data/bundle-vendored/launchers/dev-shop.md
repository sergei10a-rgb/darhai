...
2. Input line 1 is exactly `[DARHAI_CRON_FIRE:dev-shop]` → standup.
...
Scan Step 6 for any cron whose `message` line 1 is `[DARHAI_CRON_FIRE:dev-shop]`. If found, skip - already standing. Else proceed. Substitute cron if user requested different cadence:
...
message: [DARHAI_CRON_FIRE:dev-shop]
