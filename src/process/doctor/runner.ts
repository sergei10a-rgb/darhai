 ...
 *
 * Ported from upstream commit e4324b592. Every check is wrapped so a thrown
 * error (or a check that hangs forever) cannot abort the whole battery: a
 * throw becomes a `fail` result, and each check is bounded by a per-check
 * timeout. Checks run concurrently — they are independent and mostly I/O-bound
 * (fs stats, one local HTTP probe), so a serial run would needlessly multiply
 * the wall-clock time.
 */
  ...
  const timeout = new Promise<DoctorCheckResult>((resolve) => {
    timer = setTimeout(() => {
      resolve({
        ...
        remediation: 'The subsystem did not respond. Re-run, and check the relevant service.',
