...
/**
 * Update-on-quiesce gate.
 *
 * An auto-update restart must never yank the rug out from under active work.
 * Darhai runs autonomous agents, scheduled tasks and standing teams, which
 * makes it server-like: like VS Code, Slack or Chrome it should apply an
 * update on idle or on quit, never force-restart mid-task. This module wraps
 * the install call sites: if the app is busy and defer-while-busy is on, it
 * registers a one-shot install for when everything goes idle and reports
 * 'deferred'; otherwise it installs now.
 *
 * The busy signal is {@link cronBusyGuard}, which every agent manager
 * (WCore/ACP/Gemini/Remote) already feeds - and because team wakes and cron
 * runs funnel through those managers, one registry answers "is anything
 * working right now" across chat + cron + team.
 */
 ...
 * process (no IPC round-trip) so the gate can decide at the install call site.
 * Never throws - a config read failure falls back to the safe default (defer).
 */
 ...
 * another onceAllIdle, and the idle moment would fire several installs.
 */
 ...
 * @param onDeferred Optional; invoked when the install is deferred.
 */
