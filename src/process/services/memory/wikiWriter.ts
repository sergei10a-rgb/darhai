...
function formatWikiContent(entry: MemoryEntry, promotedAt: string): string {
  ...
  const lines: string[] = [
    '---',
    `promoted_from: ${entry.sourcePath}:L${entry.sourceLine}`,
...
async function rebuildWikiIndex(wikiDir: string): Promise<void> {
  ...
  const lines: string[] = [
    '# Wiki Index',
    '<!-- auto-generated by Wayland v0.6.4 -->',
    '',
  ];
...
export async function promoteEntry(
  ...
  promotionLocks.set(wikiDir, result.catch((): undefined => undefined));
