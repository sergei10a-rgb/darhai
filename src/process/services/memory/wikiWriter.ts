...
// Real running version (Electron app.getVersion(), else package.json) so the
// wiki-index stamp cannot go stale the way the hardcoded one did.
...
async function rebuildWikiIndex(wikiDir: string): Promise<void> {
  ...
  const appVersion = getPlatformServices().paths.getVersion();
  const lines: string[] = ['# Wiki Index', `<!-- auto-generated by Darhai v${appVersion} -->`, ''];
