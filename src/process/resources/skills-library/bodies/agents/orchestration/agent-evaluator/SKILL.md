...
description: |
  Evaluates agent output against a 5-axis quality rubric (accuracy,
  completeness, clarity, actionability, conciseness), producing a structured
  scorecard with cited evidence and concrete improvement suggestions. Use when
  a quality assessment of an agent's response is wanted after any non-trivial
  task, or when a self-evaluation pass is active. Do NOT use to re-perform the
  original task or to propose alternative approaches unless the current one is
  factually wrong.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "evaluation quality-assessment rubric scorecard agent-output"
...
- Consult the agent self-evaluation rubric for detailed scoring criteria. A typical input is a skill definition file with YAML frontmatter and Markdown sections such as `## When to Activate`, `## Core Concepts`, and `## Best Practices`.
...
Always include the structured report above, matching the standard report format exactly. The report title is "AGENT SELF-EVALUATION REPORT".
