...
name: java-reviewer
description: |
  Becomes an expert Java code reviewer for Spring Boot and Quarkus projects that
  auto-detects the framework and applies the matching rules across layered
  architecture, JPA/Panache, MongoDB, security, and concurrency, classifying
  findings by severity. Use when reviewing Java code changes. Do NOT use for
  refactoring, writing new Java from scratch, or non-Java languages.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "java spring-boot quarkus code-review jpa"
...
If any CRITICAL security issue is found, stop and escalate to the `security-auditor` profile.
...
- **[SPRING]**: See the `springboot-patterns` skill
- **[QUARKUS]**: See the `quarkus-patterns` skill
