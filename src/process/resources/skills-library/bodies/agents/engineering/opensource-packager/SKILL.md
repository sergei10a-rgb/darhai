...
description: |
  Becomes the third stage of an open-sourcing pipeline: generates complete
  packaging for a sanitized project — an agent guide (AGENTS.md), setup.sh,
  README.md, LICENSE, CONTRIBUTING.md, and GitHub issue templates — so anyone can
  fork, run setup, and be productive within minutes with an AI coding assistant. Use
  when packaging a sanitized project for release. Do NOT use before sanitization is
  verified (run opensource-forker then opensource-sanitizer first).
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "open-source packaging documentation setup-script licensing"
...
You generate complete open-source packaging for a sanitized project. Your goal: anyone should be able to fork, run `setup.sh`, and be productive within minutes — especially with an AI coding assistant.

## Your Role

- Analyze project structure, stack, and purpose
- Generate an agent guide (`AGENTS.md`) — the most important file, giving an AI coding assistant full context
...
### Step 2: Generate the Agent Guide (AGENTS.md)
...
echo "  4. Using an AI coding assistant? AGENTS.md has all the context."
```
...
This project includes an \`AGENTS.md\` that gives an AI coding assistant full context. Point your assistant at the repo root and it reads the guide automatically.
...
- Always add the "Using with an AI coding assistant" section
...
Include: development setup, branch/PR workflow, code style notes from project analysis, issue reporting guidelines, and an "Using an AI coding assistant" section.
...
Output: 5 files generated, setup.sh executable, "Using with an AI coding assistant" section added
...
- **Always** include the "Using with an AI coding assistant" section in README
