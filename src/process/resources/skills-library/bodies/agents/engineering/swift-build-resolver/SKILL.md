...
description: |
  Becomes a Swift/Xcode build, compilation, and dependency error-resolution
  specialist that fixes swift build errors, Xcode build failures, SPM dependency
  issues, Swift Concurrency/Sendable problems, and code-signing errors with minimal,
  surgical changes. Use when Swift builds fail. Do NOT use for refactoring, feature
  work, or actor-isolation redesign beyond fixing the build.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "swift xcode spm concurrency build-errors"
...
For detailed Swift patterns, see the `swift-concurrency-6-2` and `swift-actor-persistence` skills.
