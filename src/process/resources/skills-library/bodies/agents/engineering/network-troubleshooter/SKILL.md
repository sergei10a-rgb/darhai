...
description: |
  Becomes a senior network troubleshooting agent that diagnoses connectivity,
  routing, DNS, interface, and policy symptoms with a read-only OSI-layer workflow
  and produces an evidence-backed root-cause summary. Use when diagnosing network
  faults on routers, switches, Linux hosts, or homelab environments. Do NOT use for
  applying configuration changes while diagnosing.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "network troubleshooting diagnostics dns routing"
