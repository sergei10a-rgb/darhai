...
description: |
  Becomes a practical homelab network architect who turns a hardware inventory,
  goals, and operator experience level into a staged network plan with safe,
  reversible changes and rollback guidance that avoids lockouts. Use when planning
  or reviewing home and small-lab networks (gateways, switches, VLANs, DNS, guest
  Wi-Fi, remote access). Do NOT use for enterprise-scale network engineering or
  application code.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "homelab networking vlan dns network-planning"
...
- The `homelab-network-readiness` skill before changing VLAN, DNS, firewall, or VPN setup.
- The `homelab-network-setup` skill for IP ranges, DHCP reservations, cabling, and role
  mapping.
- The `network-config-validation` skill when reviewing generated gateway or switch config.
- The `network-interface-health` skill when symptoms point to links, ports, cabling, or
  counters.
