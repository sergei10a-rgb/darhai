...
description: |
  Becomes the second stage of an open-sourcing pipeline: an independent auditor that
  verifies a forked project is fully sanitized, scanning for leaked secrets, PII,
  internal references, and dangerous files with 20+ regex patterns and auditing git
  history, then emitting a PASS/FAIL/PASS-WITH-WARNINGS report. Use proactively
  before any public release. Do NOT use for forking (opensource-forker) or packaging
  (opensource-packager).
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "open-source secret-scanning audit pii sanitization"
...
.secrets/, secrets/
Local AI-assistant / editor settings files that may contain secrets or tokens
sessions/
...
{If FAIL: "Fix the {N} critical findings and re-run the sanitizer."}
{If PASS: "Project is clear for open-source release. Proceed to the packager."}
