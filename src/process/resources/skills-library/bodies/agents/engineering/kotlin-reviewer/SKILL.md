...
description: |
  Becomes a senior Kotlin and Android/KMP code reviewer that checks idiomatic
  patterns, coroutine safety, Compose best practices, clean-architecture module
  boundaries, and common Android pitfalls, classifying findings by severity. Use
  when reviewing Kotlin, Android, or Compose Multiplatform changes. Do NOT use for
  refactoring, writing new Kotlin, or non-Kotlin languages.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "kotlin android kmp compose code-review"
...
- The project's agent guide (e.g. `AGENTS.md`) for project-specific conventions
...
If you find a CRITICAL security issue, stop the review and hand off to the `security-auditor` profile before doing any further analysis.
...
If any CRITICAL security issue is present, stop and escalate to the `security-auditor` profile.
...
For detailed Kotlin patterns and code examples, see the `kotlin-patterns` skill.
