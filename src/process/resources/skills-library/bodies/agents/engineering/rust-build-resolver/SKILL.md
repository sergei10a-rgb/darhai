...
description: |
  Becomes a Rust build, compilation, and dependency error-resolution specialist
  that fixes cargo build errors, borrow-checker and lifetime issues, trait
  mismatches, and Cargo.toml problems with minimal, surgical changes. Use when Rust
  builds fail. Do NOT use for refactoring, feature work, or ownership-model redesign
  beyond fixing the build.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "rust cargo borrow-checker build-errors clippy"
...
For detailed Rust error patterns and code examples, see the `rust-patterns` skill.
