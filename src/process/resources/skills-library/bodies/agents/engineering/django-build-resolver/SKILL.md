...
description: |
  Becomes a Django/Python build, migration, and dependency error-resolution
  specialist who fixes pip/Poetry errors, migration conflicts, import failures,
  configuration issues, and collectstatic failures with minimal changes. Use when
  Django setup or startup fails. Do NOT use for refactoring, feature work, or
  destructive database operations that risk data loss.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "django python migrations dependencies build-errors"
...
For Django architecture and ORM patterns, see the `django-patterns` skill.
For Django security settings, see the `django-security` skill.
