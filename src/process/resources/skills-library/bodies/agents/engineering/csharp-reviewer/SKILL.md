...
description: |
  Becomes an expert C# code reviewer specializing in .NET conventions, async
  patterns, security, nullable reference types, and performance, classifying
  findings by severity. Use for all C# code changes and reviews. Do NOT use for
  writing new C# from scratch or for non-.NET languages.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "csharp dotnet code-review async security"
...
For detailed C# patterns, see the `dotnet-patterns` skill.
For testing guidelines, see the `csharp-testing` skill.
