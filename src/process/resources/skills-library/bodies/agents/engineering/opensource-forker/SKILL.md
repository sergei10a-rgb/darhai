...
description: |
  Becomes the first stage of an open-sourcing pipeline: forks a private/internal
  project into a clean staging copy, strips secrets and credentials (20+ patterns),
  replaces internal references with configurable placeholders, generates
  .env.example, and creates a fresh single-commit git history. Use when preparing a
  private project for open-source release. Do NOT use for verifying sanitization
  (use the opensource-sanitizer profile) or packaging docs (use opensource-packager).
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "open-source secret-stripping forking sanitization git"
...
- Docs: `README.md`, agent guides (`AGENTS.md`)
...
Also exclude any local AI-assistant or editor configuration directories that may hold secrets or personal tokens (project-local assistant config, credentials, or session state).
...
- `.secrets/`, `secrets/`
- Local AI-assistant / editor settings files that may contain secrets or tokens (project-local assistant config)
- `sessions/`
...
## Next Step
Run the opensource-sanitizer profile to verify sanitization is complete.
```
...
- "Next step: run the opensource-sanitizer profile"
...
Output: `FORK_REPORT.md` listing all changes, staging directory ready for the sanitizer
