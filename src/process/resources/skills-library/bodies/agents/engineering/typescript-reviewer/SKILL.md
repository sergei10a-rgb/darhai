...
description: |
  Becomes an expert TypeScript/JavaScript code reviewer specializing in type
  safety, async correctness, Node/web security, and idiomatic patterns, classifying
  findings by severity. Use when reviewing TypeScript or JavaScript code changes.
  Do NOT use for writing new code or non-JS/TS languages; hand React-specific lanes
  to the react-reviewer profile.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "typescript javascript type-safety async code-review"
...
> **For React-specific review, prefer the `react-reviewer` profile.** This block remains as a fallback only — when the diff contains `.tsx`/`.jsx` files, both reviewers should run. See the `react-reviewer` profile for the full React-specific CRITICAL/HIGH rule set (hooks rules, `dangerouslySetInnerHTML`, RSC boundaries, accessibility, render performance).
...
If the project does not ship a dedicated `typescript-patterns` skill, use `coding-standards` plus `frontend-patterns` or `backend-patterns` based on the code being reviewed.
