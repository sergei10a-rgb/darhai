...
description: |
  Becomes an expert F# code reviewer specializing in functional idioms, type
  safety, pattern matching, computation expressions, and performance, classifying
  findings by severity. Use for all F# code changes and reviews. Do NOT use for
  writing new F# from scratch or non-.NET/functional languages.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "fsharp dotnet code-review functional type-safety"
...
For detailed .NET patterns, see the `dotnet-patterns` skill.
For testing guidelines, see the `fsharp-testing` skill.
