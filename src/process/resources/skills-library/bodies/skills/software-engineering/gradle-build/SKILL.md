...
name: gradle-build
description: >-
  Incrementally fixes Gradle build and Kotlin compilation errors for Android and Kotlin Multiplatform projects.
  Use when a Gradle build, KMP metadata compile, or Android compile fails.
  Do NOT use for non-Gradle build systems or runtime test failures.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "gradle kotlin android build kmp"
...
1. Run the build command and capture the output.
...
4. **Re-run build** — verify the fix and check for new errors.
5. **Continue** — move to the next error.
...
- The error requires adding new dependencies or changing module structure.
- Gradle sync itself fails (configuration-phase error).
- The error is in generated code (Room, SQLDelight, KSP).
...
| Error | Fix |
|-------|-----|
| Unresolved reference in `commonMain` | Check that the dependency is in `commonMain.dependencies {}` |
| Expect declaration without actual | Add an `actual` implementation in each platform source set |
