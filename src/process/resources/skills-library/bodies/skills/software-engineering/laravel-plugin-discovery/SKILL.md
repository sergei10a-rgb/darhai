...
description: |
  Find, evaluate, and choose healthy Laravel/PHP packages using the public LaraPlugins.io MCP server — search by keyword, health band, vendor, and Laravel/PHP version compatibility, and assess package health before adding a dependency.
  Use when the user wants to find a Laravel package for a feature, check whether a package is actively maintained, or verify version compatibility.
  Do NOT use for non-Laravel package ecosystems or for writing application code.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "laravel php packages dependency-discovery developer-tools"
...
The LaraPlugins MCP server must be configured. It is a free public HTTP MCP server — no API key required and no local process to run. Register it in your host's MCP configuration using its HTTP endpoint:
...
- Laravel architecture and patterns guidance
