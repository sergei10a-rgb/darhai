...
description: |
  Retrieve, analyze, and update Jira tickets from a coding workflow via the mcp-atlassian MCP server (recommended) or the Jira REST API v3 directly — fetch requirements, extract testable acceptance criteria, add comments, transition issues, and link PRs.
  Use when fetching a Jira ticket, extracting acceptance criteria, updating ticket status, or searching issues by JQL.
  Do NOT use for non-Jira trackers, and never hardcode API tokens.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "jira issue-tracking mcp requirements developer-tools"
...
Retrieve, analyze, and update Jira tickets directly from your coding workflow. Supports both **MCP-based** (recommended) and **direct REST API** approaches.
...
Install the `mcp-atlassian` MCP server. This exposes Jira tools directly to your agent.
...
| `spawn uvx ENOENT` | Host cannot find `uvx` on PATH | Use the full path (e.g. `~/.local/bin/uvx`) or add it to PATH in your shell profile |
