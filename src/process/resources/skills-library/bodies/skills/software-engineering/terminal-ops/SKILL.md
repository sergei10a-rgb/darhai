...
name: terminal-ops
description: |
  An evidence-first repo execution workflow: run commands, inspect git state, debug CI or build failures, make a narrow fix, and report exactly what changed and what was verified — distinguishing "changed locally", "verified locally", "committed", and "pushed".
  Use when the user wants a command run, a repo checked, a CI failure debugged, or a narrow fix pushed with exact proof of what was executed and verified.
  Do NOT use for broad feature planning, large refactors, or work that does not touch a real repository/terminal.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "terminal git ci-debugging evidence-first developer-tools"
...
Pull these capabilities into the workflow when relevant:

- A verification pass for exact proving steps after changes
- A TDD workflow when the right fix needs regression coverage
- A security review when secrets, auth, or external inputs are involved
- A VCS/CI capability when the task depends on CI runs, PR state, or release status
- A durable-context capability when the verified outcome needs to be captured for later
