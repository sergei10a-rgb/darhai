...
description: |
  Systematically analyze an unfamiliar codebase and produce a structured onboarding guide with an architecture map, key entry points, conventions, and a starter host-instructions file (AGENTS.md).
  Use when joining a new project, taking over a repo, or setting up an AI coding agent in an existing repository for the first time.
  Do NOT use for deep bug investigation, single-file edits, or when comprehensive project docs already exist and are current.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "onboarding codebase-analysis architecture conventions developer-tools"
...
Systematically analyze an unfamiliar codebase and produce a structured onboarding guide. Designed for developers joining a new project or setting up an AI coding agent in an existing repo for the first time.
...
- User asks to generate a host-instructions file (e.g. `AGENTS.md`) for a project
...
#### Output 2: Starter Host-Instructions File

Generate or update a project-specific host-instructions file (commonly `AGENTS.md`) based on detected conventions. If one already exists, read it first and enhance it — preserve existing project-specific instructions and clearly call out what was added or changed.
...
3. **Respect existing instructions** — if a host-instructions file already exists, enhance it rather than replacing it. Call out what's new vs existing.
...
- Generating a host-instructions file that's longer than 100 lines — keep it focused
...
**Action**: Run full 4-phase workflow → produce Onboarding Guide + Starter host-instructions file
**Output**: Onboarding Guide printed directly to the conversation, plus an `AGENTS.md` written to the project root

### Example 2: Generate a host-instructions file for an existing project
**User**: "Generate an AGENTS.md for this project"
**Action**: Run Phases 1-3, skip Onboarding Guide, produce only the host-instructions file
**Output**: Project-specific `AGENTS.md` with detected conventions

### Example 3: Enhance existing instructions
**User**: "Update the project instructions with current conventions"
**Action**: Read the existing host-instructions file, run Phases 1-3, merge new findings
**Output**: Updated instructions with additions clearly marked
