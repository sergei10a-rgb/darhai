...
name: browser-qa
description: |
  Automated post-deploy visual, interaction, and accessibility verification driven through a browser-automation tool — smoke tests (console/network/Core Web Vitals), interaction tests, visual regression against baselines, and WCAG checks — with an explicit blast-radius safety default.
  Use after deploying a feature to staging/preview, when verifying UI behavior across pages, or before shipping frontend changes.
  Do NOT run mutating journeys against production, and do NOT report "accessible" from an automated pass alone.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "browser-testing visual-regression accessibility smoke-test test-automation"
...
Uses a browser-automation tool (a browser-driving MCP, Playwright, or Puppeteer) to interact with live pages like a real user.
...
- A browser-driving MCP (preferred — uses a real browser)
- Playwright or Puppeteer scripts
- Any headless-browser harness that supports navigation, screenshots, and JS evaluation

Pair with a post-deploy monitoring/canary workflow for ongoing coverage.
