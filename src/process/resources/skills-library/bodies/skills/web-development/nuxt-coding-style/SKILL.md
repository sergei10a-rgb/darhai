...
description: >-
  nuxt coding style conventions and rules. Use when writing or reviewing nuxt code for coding style.
  - Default `srcDir` is `app/`. Framework files live at `app/pages/`, `app/layouts/`,
  `app/middleware/`, `app/plugins/`, `app/app.config.ts`. `nuxt.config.ts` and `server/` stay at p
license: Apache-2.0
metadata:
  author: darhai
  version: 1.0.0
  tags: nuxt coding-style standards best-practices
