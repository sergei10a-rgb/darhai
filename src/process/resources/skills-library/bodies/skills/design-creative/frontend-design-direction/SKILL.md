...
description: |
  Set a specific, product-appropriate frontend design direction before coding — purpose, audience, tone, one memorable detail, and constraints — then apply implementation and review discipline so the UI feels intentional rather than generic or templated.
  Use when building or improving websites, dashboards, applications, components, landing pages, or any web UI that needs stronger product-specific design judgment.
  Do NOT use for pure backend work, or when the task is only to make an existing UI function without any visual-quality concern.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "ui-ux design-direction frontend visual-hierarchy web-design"
