...
name: knowledge-ops
description: |
  Manage a multi-layered knowledge system for ingesting, organizing, syncing, deduplicating, and retrieving knowledge across storage layers — local memory files, an MCP memory graph, a durable knowledge-base repo, an external data store, and issue/project trackers.
  Use when the user wants to save, organize, sync, deduplicate, or search across their knowledge systems.
  Do NOT use for simple single-fact memory recall or for live application-database CRUD.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "knowledge-management note-taking memory sync organization"
...
- active execution context lives in your issue/project tracker and repo-local working-context files
...
- Syncing knowledge across systems (local files, MCP memory, external store, Git repos)
...
- **Sources:** issue tracker (issues, PRs, discussions, release notes), project tracker (projects/docs)
...
### Layer 2: Agent memory (quick access)
- **Path:** the agent's project memory directory
...
- **Access:** MCP memory tools (create-entities, create-relations, add-observations, search-nodes)
...
- **Use for:** human-facing notes, archived plans, local media organization, temporary non-code docs
...
- Active roadmap / release / implementation state -> issue + project tracker first
...
- Check whether the information already exists in the issue/project tracker before creating another local note
- Do not create duplicates. Update existing entries instead.

### 3. Store
Write to appropriate layer(s):
- Always update agent memory for quick access
- Use MCP memory for semantic searchability and relationship mapping
- Update the issue/project tracker first when the information changes live project truth
...
- Sources: agent session files from any assistant
...
- update the relevant issue, PR, discussion, release notes, or roadmap thread
- attach supporting docs to the project tracker when the work needs durable planning context
...
- Assistant conversation exports
- Browser bookmarks
- VCS activity events
...
Write to the agent's project memory directory for cross-session recall

# Long-term: tracker / KB
Put active execution truth in the issue + project tracker
...
Use search-nodes to find existing knowledge
```
