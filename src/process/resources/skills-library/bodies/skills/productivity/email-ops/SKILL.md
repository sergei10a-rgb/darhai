...
name: email-ops
description: |
  An evidence-first mailbox operator workflow: triage, draft-first composition, send verification against the Sent folder, and account-disciplined follow-up through the real mail surface.
  Use when the task is mailbox work — triage, drafting, replying, sending, or proving a message actually landed in Sent.
  Do NOT use as a generic writing skill, and do NOT claim a message was sent without a Sent-folder or client-side confirmation.
license: Apache-2.0
metadata:
  author: darhai
  version: "1.0.0"
  tags: "email inbox-management triage communication organization"
...
- A brand/voice guide before drafting anything user-facing
- An outreach playbook for investor, partner, or sponsor-facing mail
- A billing/support-ops capability when the thread is a billing or support incident rather than generic correspondence
- A durable-context capability when the message or thread should be captured for later
- A research capability when a reply depends on fresh external facts
...
- if the task is really DM or text-message work, hand it off to a messaging workflow
...
- pull the brand/voice guidance before drafting
