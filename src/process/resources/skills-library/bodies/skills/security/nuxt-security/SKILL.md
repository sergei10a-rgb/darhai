...
name: nuxt-security
description: >-
  nuxt security conventions and rules. Use when writing or reviewing nuxt code for security. - Root
  `runtimeConfig` keys are server-only. `runtimeConfig.public` serializes into EVERY page payload
  (client-visible).
license: Apache-2.0
metadata:
  author: darhai
  version: 1.0.0
  tags: nuxt security standards best-practices
  category: security
