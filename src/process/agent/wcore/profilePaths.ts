 ...
 *
 * The variable keeps the ENGINE's name because the bundled engine binary is
 * upstream's: it reads `WAYLAND_HOME` and has never read `DARHAI_HOME`. See the
 * measurement in `envBuilder.ts`. Renaming it here would silently disable
 * profile isolation, which is what the earlier `DARHAI_HOME` spelling did.
 *
 ...
 *
 * All three branches are measured against the bundled binary in
 * `tests/unit/wcore-engineHome.test.ts`, so this mirror cannot drift from the
