...
export const autoUpdate = {
  ...
  /**
   * Quit and install the downloaded update. Routed through the
   * update-on-quiesce gate: if the app is busy and defer-while-busy is on, the
   * restart is DEFERRED until idle (status becomes 'deferred') instead of
   * killing in-flight work. Pass `{ force: true }` to bypass the gate
   * ("install now anyway").
   */
