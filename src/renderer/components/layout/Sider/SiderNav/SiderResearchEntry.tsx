...
interface SiderResearchEntryProps {
  isMobile: boolean;
  isActive: boolean;
  collapsed: boolean;
...
const SiderResearchEntry: React.FC<SiderResearchEntryProps> = ({ isActive, collapsed, siderTooltipProps, onClick }) => {
  const { t } = useTranslation();
  const label = t('research.sidebarLabel');
    ...
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        <div
          ...
          onClick={onClick}
          data-testid='sider-research-entry'
        >
          <Telescope size={20} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-28px h-28px flex items-center justify-center shrink-0'>
          <Telescope size={20} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
...
export default SiderResearchEntry;
