...
interface SiderCompareEntryProps {
  isMobile: boolean;
  isActive: boolean;
  collapsed: boolean;
...
const SiderCompareEntry: React.FC<SiderCompareEntryProps> = ({ isActive, collapsed, siderTooltipProps, onClick }) => {
  const { t } = useTranslation();
  const label = t('compare.sidebarLabel');
    ...
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        <div
          ...
          onClick={onClick}
          data-testid='sider-compare-entry'
        >
          <Columns3 size={20} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-28px h-28px flex items-center justify-center shrink-0'>
          <Columns3 size={20} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
...
export default SiderCompareEntry;
