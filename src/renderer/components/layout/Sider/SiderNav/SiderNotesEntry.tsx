...
interface SiderNotesEntryProps {
  isMobile: boolean;
  isActive: boolean;
  collapsed: boolean;
...
const SiderNotesEntry: React.FC<SiderNotesEntryProps> = ({ isActive, collapsed, siderTooltipProps, onClick }) => {
  const { t } = useTranslation();
  const label = t('notes.sidebarLabel');
    ...
    return (
      <Tooltip {...siderTooltipProps} content={label} position='right'>
        <div
          ...
          onClick={onClick}
          data-testid='sider-notes-entry'
        >
          <NotebookPen size={20} className='block leading-none shrink-0' style={{ lineHeight: 0 }} />
        </div>
      </Tooltip>
    );
  }
    ...
    <Tooltip {...siderTooltipProps} content={label} position='right'>
      ...
      >
        <span className='w-28px h-28px flex items-center justify-center shrink-0'>
          <NotebookPen size={20} className='block leading-none' style={{ lineHeight: 0 }} />
        </span>
...
export default SiderNotesEntry;
