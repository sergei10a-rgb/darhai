...
import { shouldInjectMcpServer } from '../../src/process/agent/acp/mcpSessionConfig';
...
describe('shouldInjectMcpServer', () => {
  it('injects an enabled builtin server with status undefined (the bug)', () => {
    expect(shouldInjectMcpServer(server({ builtin: true, enabled: true, status: undefined }))).toBe(true);
  });

  it('injects an enabled builtin server with status connected', () => {
    expect(shouldInjectMcpServer(server({ builtin: true, enabled: true, status: 'connected' }))).toBe(true);
  });

  it('does not inject a disabled builtin server', () => {
    expect(shouldInjectMcpServer(server({ builtin: true, enabled: false, status: undefined }))).toBe(false);
  });

  it('does not inject a builtin server in an error status', () => {
    expect(shouldInjectMcpServer(server({ builtin: true, enabled: true, status: 'error' }))).toBe(false);
  });

  it('requires a user server to be connected (undefined is not injected)', () => {
    expect(shouldInjectMcpServer(server({ builtin: undefined, enabled: true, status: undefined }))).toBe(false);
  });

  it('injects a connected user server', () => {
    expect(shouldInjectMcpServer(server({ builtin: undefined, enabled: true, status: 'connected' }))).toBe(true);
  });

  it('does not inject a disabled user server even when connected', () => {
    expect(shouldInjectMcpServer(server({ builtin: undefined, enabled: false, status: 'connected' }))).toBe(false);
  });
});
