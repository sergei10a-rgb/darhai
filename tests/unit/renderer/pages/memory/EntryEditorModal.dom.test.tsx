...
/**
 * DOM tests for EntryEditorModal (#414 port). The load-bearing behavior is the
...
// summary/body inputs; other fields are inert passthroughs for this diff test.
...
describe('EntryEditorModal (#414 port)', () => {
  ...
  it('shows an inline warning while the body contains a lone --- line, and clears it (C1)', () => {
    render(<EntryEditorModal open entry={ENTRY} onClose={vi.fn()} onSaved={vi.fn()} />);
    expect(screen.queryByTestId('entry-editor-body-separator-warn')).toBeNull();

    fireEvent.change(screen.getByTestId('entry-editor-body'), { target: { value: 'line one\n---\nline two' } });
    expect(screen.getByTestId('entry-editor-body-separator-warn')).toBeTruthy();

    fireEvent.change(screen.getByTestId('entry-editor-body'), { target: { value: 'line one\nline two' } });
    expect(screen.queryByTestId('entry-editor-body-separator-warn')).toBeNull();
  });

  it('does not warn for dashes that are not a lone separator line (C1)', () => {
    render(<EntryEditorModal open entry={ENTRY} onClose={vi.fn()} onSaved={vi.fn()} />);
    fireEvent.change(screen.getByTestId('entry-editor-body'), { target: { value: 'a ---\n----\nfoo---bar' } });
    expect(screen.queryByTestId('entry-editor-body-separator-warn')).toBeNull();
  });

  it('surfaces the typed body_contains_separator refusal with its own message (C1)', async () => {
    updateEntry.mockResolvedValue({ ok: false, error: 'body_contains_separator' });
    render(<EntryEditorModal open entry={ENTRY} onClose={vi.fn()} onSaved={vi.fn()} />);
    fireEvent.change(screen.getByTestId('entry-editor-body'), { target: { value: 'a\n---\nb' } });
    fireEvent.click(screen.getByTestId('modal-ok'));
    await waitFor(() => expect(updateEntry).toHaveBeenCalledTimes(1));
    await waitFor(() =>
      expect(vi.mocked(Message.error)).toHaveBeenCalledWith('memory.archive.editor.toastBodySeparator')
    );
  });

  it('accepts Cyrillic edits and sends them unmangled', async () => {
    render(<EntryEditorModal open entry={ENTRY} onClose={vi.fn()} onSaved={vi.fn()} />);
    fireEvent.change(screen.getByTestId('entry-editor-summary'), {
      target: { value: 'Кэш стратегийг Redis рүү шилжүүлэв' },
    });
    fireEvent.click(screen.getByTestId('modal-ok'));
    await waitFor(() =>
      expect(updateEntry).toHaveBeenCalledWith({ id: 'entry-001', summary: 'Кэш стратегийг Redis рүү шилжүүлэв' })
    );
  });
});
