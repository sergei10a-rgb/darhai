...
/**
 * Pure block-transform tests for memory entry edit/delete (ported from
 * upstream f55f934b6, #414). The transforms must be byte-preserving for every
 * block that is NOT the target, surgical for the target's frontmatter, and
 * must fail closed on ambiguity. Mongolian Cyrillic content is exercised
 * explicitly because Darhai's memory corpus is Cyrillic-first.
 */

import { describe, expect, it } from 'vitest';
import { applyDelete, applyEdit, serializeScalar } from '@process/services/memory/memoryEntryMutation';
...
describe('memoryEntryMutation.applyEdit', () => {
  ...
  it('round-trips Mongolian Cyrillic summary, tags and body through the parser', () => {
    const r = applyEdit(TWO_ENTRY, 'First entry summary', {
      summary: 'Кэш стратегийг Redis рүү шилжүүлсэн шийдвэр',
      tags: ['шийдвэр', 'кэш'],
      body: 'Бүх session кэшийг Redis рүү нүүлгэв.\n**Why:** локал кэш restart бүрт алдагддаг.',
    });
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    const blocks = parseMarkdownBlocks(r.content);
    const edited = blocks.find(
      (b) => (b.frontmatter['summary'] as string) === 'Кэш стратегийг Redis рүү шилжүүлсэн шийдвэр'
    );
    expect(edited).toBeTruthy();
    expect(edited?.frontmatter['tags']).toEqual(['шийдвэр', 'кэш']);
    expect(edited?.body).toContain('Бүх session кэшийг Redis рүү нүүлгэв.');
...
describe('memoryEntryMutation - body containing a lone `---` line (C1 data-loss guard)', () => {
  // The adversarial repro: writing a body with a bare `---` line makes the
  // block state machine read that line as an entry separator, so the NEXT
  // entry's frontmatter (summary/type/stored/tags) is swallowed into a phantom
  // block. Both transforms must refuse instead of corrupting the file.

  it('applyEdit refuses a body with a lone --- line (would corrupt the next entry)', () => {
    const r = applyEdit(TWO_ENTRY, 'First entry summary', { body: 'line one\n---\nline two' });
    expect(r).toEqual({ ok: false, error: 'body_contains_separator' });
  });

  it('applyEdit refuses `---` with trailing whitespace and CRLF variants too', () => {
    expect(applyEdit(TWO_ENTRY, 'First entry summary', { body: 'a\n---   \nb' })).toEqual({
      ok: false,
      error: 'body_contains_separator',
    });
    expect(applyEdit(TWO_ENTRY, 'First entry summary', { body: 'a\r\n---\r\nb' })).toEqual({
      ok: false,
      error: 'body_contains_separator',
    });
    expect(applyEdit(TWO_ENTRY, 'First entry summary', { body: '---' })).toEqual({
      ok: false,
      error: 'body_contains_separator',
    });
  });

  it('applyEdit still allows dashes that are NOT a lone separator line', () => {
    const r = applyEdit(TWO_ENTRY, 'First entry summary', { body: 'a ---\n----\nfoo---bar\n- --' });
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    // The sibling entry survives the round-trip intact.
    const blocks = parseMarkdownBlocks(r.content);
    expect(blocks).toHaveLength(2);
    expect(blocks[1]?.frontmatter['summary']).toBe('Second entry summary');
  });

  it('applyDelete refuses when the FILE already contains a body-split separator (positions unreliable)', () => {
    // A file corrupted by an earlier writer: entry one's body contains `---`,
    // so shapeFile mis-counts blocks and any computed block range is a guess.
    const corrupted = [
      '---',
      'type: observation',
      'summary: First entry summary',
    ...
    const r = applyDelete(corrupted, 'First entry summary');
    expect(r).toEqual({ ok: false, error: 'body_contains_separator' });
  });

  it('applyDelete still deletes normally from a well-formed file', () => {
    const r = applyDelete(TWO_ENTRY, 'First entry summary');
    expect(r.ok).toBe(true);
  });
});
...
describe('serializeScalar', () => {
  it('leaves plain values unquoted', () => {
    ...
    expect(serializeScalar('alpha-beta')).toBe('alpha-beta');
    // Plain Cyrillic needs no quoting either.
    expect(serializeScalar('шийдвэр')).toBe('шийдвэр');
  });
