...
describe('WebhookAdapter.signOutboundBody — HMAC signing', () => {
  ...
  it('returns sha256=<hex> header value matching manual HMAC', () => {
    const sig = signOutboundBody(bodyJson, secret);
    expect(sig).not.toBeNull();
    const expected = 'sha256=' + createHmac('sha256', secret).update(bodyJson, 'utf8').digest('hex');
    expect(sig).toBe(expected);
  });

  it('signature changes when body changes', () => {
    const sig1 = signOutboundBody(bodyJson, secret);
    const sig2 = signOutboundBody(bodyJson + 'x', secret);
    expect(sig1).not.toBe(sig2);
  });

  it('signature changes when secret changes', () => {
    const sig1 = signOutboundBody(bodyJson, secret);
    const sig2 = signOutboundBody(bodyJson, 'different-secret');
    expect(sig1).not.toBe(sig2);
  });

  it('returns null when secret is empty string', () => {
    expect(signOutboundBody(bodyJson, '')).toBeNull();
  });
