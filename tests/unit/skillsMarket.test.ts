 ...
 *
 * Tests the enable/disable flow for the wayland-skills builtin skill:
...
const BUNDLED_SKILL_PATH = path.resolve(
  __dirname,
  '../../src/process/resources/skills/_builtin/wayland-skills/SKILL.md'
);

// Mock Electron app and initStorage before importing AcpSkillManager
vi.mock('electron', () => ({ app: { setName: vi.fn(), getPath: () => '/tmp/wayland-test' } }));
vi.mock('../../src/process/utils/initStorage', () => ({
  getSkillsDir: () => path.join('/tmp/wayland-test', 'skills'),
  getAutoSkillsDir: () => path.join('/tmp/wayland-test', 'skills', '_builtin'),
  getBuiltinSkillsCopyDir: () => path.join('/tmp/wayland-test', 'builtin-skills'),
}));
...
describe('Skills Market - Bundled SKILL.md', () => {
  ...
  it('has valid frontmatter with name and description', async () => {
    ...
    expect(nameMatch![1].trim()).toBe('wayland-skills');

    const descMatch = frontmatter.match(/^description:\s*(.+)$/m);
    expect(descMatch).not.toBeNull();
    expect(descMatch![1]).toContain('Wayland Skills');
  });

  it('contains the curl command for fetching full SKILL.md', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('curl -s https://skills.wayland.app/SKILL.md');
  });
  ...
  it('references the standard credentials path', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('~/.config/wayland-skills');
  });
  ...
  it('does NOT contain the full API documentation', async () => {
    ...
    expect(content).not.toContain('X-Wayland-Skills-Checksum');
  });
});
...
describe('Skills Market - Enable/Disable flow', () => {
  ...
  it('enable: creates wayland-skills directory with SKILL.md', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'wayland-skills');
    ...
    expect(written).toContain('name: wayland-skills');
  });

  it('disable: removes wayland-skills directory completely', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'wayland-skills');
  ...
  it('disable: fs.rm with force does not throw if directory does not exist', async () => {
    const skillDir = path.join(tmpDir, '_builtin', 'wayland-skills');
