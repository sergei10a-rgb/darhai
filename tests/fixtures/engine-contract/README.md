...
gap-matrix.json      generated by `node scripts/gapMatrix.mjs --write` - what Darhai implements
                     vs what the contract defines. Never hand-edited: it said every
                     capability was unimplemented for a whole wave because nothing
                     regenerated it. `tests/unit/engineContractGapMatrix.test.ts` fails
                     when the checked-in file and the code disagree.
```
